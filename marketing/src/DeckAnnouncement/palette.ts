...
export const paletteFor = (deck: DeckPromo, backdropColour?: string): Palette => {
  ...
  // same contrast contract. Decks that are already dark are untouched. A props
  // `backdropColour` swaps in a different raw base entirely (same pull-down);
  // it owns the video backdrop without touching the deck's in-game border.
  const raw = backdropColour ?? deck.borderColour;
