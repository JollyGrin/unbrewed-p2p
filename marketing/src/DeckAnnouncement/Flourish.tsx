 ...
 *
 * All the maths is in `particles.ts`; this file only turns it into divs. Two
 ...
 * - **Subtle.** Nothing here is above ~0.2 opacity, and `ambientDim` pulls the
 *   field down further under every beat the viewer is reading. Seasoning, not a
 *   screensaver.
 */
...
const AmbientField: React.FC<{
  ...
  hasQuote: boolean;
}> = ({ palette, style, timeline, hasQuote }) => {
...
}> = ({ palette, style, timeline, cardAnchors }) => {
  const frame = useCurrentFrame();
